...
class movieService {
    async getAll(userId) {
        try {
            ...
            if (user.watchList && user.watchList.length > 0) {
                ...
                const recentMovies = await movieRepository.getMoviesByIds(recentMovieIds);
                response["Recently Watched"] = this._getRandomMovies(recentMovies, recentMovies.length);
            }

            // Promoted categories
            const promotedCategories = await Category.find({ promoted: true });
            const watchedMoviesIds = new Set(user.watchList.map(id => id.toString()));
    ...
    async create(movieData) {
        try {
            ...
            try {
                ...
                // If adding movie to categories fails, delete the movie
                await Movie.findByIdAndDelete(movie._id);
                throw new Error("Failed to update categories. Movie creation rolled back.");
            }
    ...
    async update(id, newData) {
        ...
        if (newData.categories) {
            ...
            for (const categoryId of categoriesToRemoveFrom) {
                ...
                if (category) {
                    category.movies = category.movies.filter(movieId => movieId.toString() !== id);
                    await categoryRepository.update(categoryId, { movies: category.movies });
                }
            }

            const categoriesToAddTo = newData.categories.filter(
                newCatId => !existingMovie.categories.map(cat => cat.toString()).includes(newCatId)
            );
    ...
    async removeFromSources(id) {
        ...
        for (const user of users) {
            user.watchList = user.watchList.filter(movieId => movieId.toString() !== id.toString());
            await user.save();
            //delete movie from recommendations Service
            try {
                await recommendationService.deleteFromWatchList(user._id.toString(), id);
                console.log(`Deleted movie ${id} from recommendations for user ${user._id}`);
            } catch (error) {
                console.log(`Failed to delete movie ${id} from recommendations for user ${user._id}: ${error.message}`);
            }
        }
